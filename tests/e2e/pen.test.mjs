...
const hCur0 = await curOf('.drawghost__hdot')
await page.keyboard.down('Alt')
await page.waitForTimeout(120)
ok((await curOf('.drawghost__anchor')) !== aCur, '⌥ → 앵커 변환 커서')
ok((await curOf('.drawghost__hdot')) !== hCur0, '⌥ → 핸들 한쪽 수정 커서')
...
// ⌥클릭 재차 = 코너 → 스무스, 살짝 당겨진 핸들 (0길이는 잡기 어려움)
ab = await (await page.$$('.drawghost__anchor'))[1].boundingBox()
await page.keyboard.down('Alt')
await page.mouse.click(ab.x + ab.width / 2, ab.y + ab.height / 2)
await page.keyboard.up('Alt')
await page.waitForTimeout(1300)
k1 = shOf(await src())
const hoLen = Math.hypot(k1.o[1][0], k1.o[1][1])
const hiLen = Math.hypot(k1.i[1][0], k1.i[1][1])
ok(hoLen >= 7 && hiLen >= 7, `⌥클릭 토글 → 시작 핸들 생성 (ho ${hoLen.toFixed(1)}, hi ${hiLen.toFixed(1)})`)
// 원상복구 — 이후 케이스는 코너 전제
await page.keyboard.down('Alt')
await page.mouse.click(ab.x + ab.width / 2, ab.y + ab.height / 2)
await page.keyboard.up('Alt')
...
await page.keyboard.up('Shift')
await page.waitForTimeout(200)
ok((await page.locator('.drawghost__anchor--sel').count()) === 0, '⇧마키 → 선택된 2개 토글 해제')
// 삭제 테스트용 재선택 — 앵커 클릭 후 마키
const a2c = await anchorBox(2)
await page.mouse.click(a2c.x + a2c.width / 2, a2c.y + a2c.height / 2)
await page.waitForTimeout(150)
await page.mouse.move(a2.x - 30, a2.y - 30)
await page.mouse.down()
await page.mouse.move(a3.x + a3.width + 30, a3.y + a3.height + 30, { steps: 5 })
