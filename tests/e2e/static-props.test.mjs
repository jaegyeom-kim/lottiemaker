...
// ① 스케일 — s 키 보유 채널 → 재생헤드 키 갱신, 중복 없이
const scaleSlider = page.locator('.knob', { hasText: /^스케일/ }).first().locator('input[type=range]')
await scaleSlider.fill('180')
await scaleSlider.dispatchEvent('change')
...
// ③ 불투명도 — o 키 없음 → 정적, 키 금지
const oSlider = page.locator('.knob', { hasText: /^불투명도/ }).first().locator('input[type=range]')
await oSlider.fill('55')
await oSlider.dispatchEvent('change')
await page.waitForTimeout(1300)
d = await src()
...
const rotSlider = page.locator('.knob', { hasText: /^회전/ }).first().locator('input[type=range]')
await rotSlider.fill('90')
