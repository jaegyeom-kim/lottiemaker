...
// ── 포인트 수 불일치 가드 — 45f 키에서 점 삭제 → ⚠ → 포인트 수 맞추기 ──
await scrubTo(45)
await page.locator('.drawbar button[title*="펜"]').click()
await page.waitForTimeout(400)
const aDel = await (await page.$$('.drawghost__anchor'))[1].boundingBox()
await page.mouse.click(aDel.x + aDel.width / 2, aDel.y + aDel.height / 2)
await page.waitForTimeout(150)
await page.keyboard.press('Backspace')
await page.waitForTimeout(1300)
d = await src()
const cnts = pkKeysOf(d).map((k) => k.pk.v.length)
ok(cnts[0] !== cnts[1], `점 삭제 → 키 간 포인트 수 불일치 (${cnts.join(' vs ')})`)
await page.keyboard.press('v')
await page.waitForTimeout(300)
const warn = page.locator('.knob__note--warn')
ok((await warn.count()) === 1, '⚠ 불일치 경고 표시')
await warn.locator('.linkbtn').click()
await page.waitForTimeout(1300)
d = await src()
const cnts2 = pkKeysOf(d).map((k) => k.pk.v.length)
ok(cnts2[0] === cnts2[1] && cnts2[0] === Math.max(...cnts), `포인트 수 맞추기 → 동일 (${cnts2.join(',')})`)
ok((await page.locator('.knob__note--warn').count()) === 0, '경고 해제')
ok(shOf(d).a === 1, '모핑 유지')
